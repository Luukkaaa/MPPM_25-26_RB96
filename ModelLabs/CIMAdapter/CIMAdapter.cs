	...
	public class CIMAdapter
	{
		...
		private bool DoTransformAndLoad(Assembly assembly, ConcreteModel concreteModel, SupportedProfiles extractType, out Delta nmsDelta, out string log)
		{
			...
			try
			{
				...
				switch (extractType)
				{
					case SupportedProfiles.Projekat96:
						{
							TransformAndLoadReport report = Projekat96Importer.Instance.CreateNMSDelta(concreteModel);

							if (report.Success)
							{
								nmsDelta = Projekat96Importer.Instance.NMSDelta;
								success = true;
							}
							else
							{
							...
							Projekat96Importer.Instance.Reset();
