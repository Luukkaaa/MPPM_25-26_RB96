	...
	public class Program
	{
			...
			try
			{
				...
				do
				{
                    ...
                    else if (str == "4")
                    {
                        ...
                        {
                            message = string.Format("Test ApplyUpdate: Insert - Update - Delete failed. {0}", ex.Message);
                            Console.WriteLine(message);
                            CommonTrace.WriteTrace(CommonTrace.TraceError, message);
                        }
                    }
        ...
        private static ModelCode InputModelCode()
        {
            ...
            try
            {
                ...
                {                 
                    if (userModelCode.StartsWith("0x", StringComparison.OrdinalIgnoreCase))
                    {
        ...
        private static Association InputAssociation()
        {
            ...
            try
            {
                ...
                if (!ModelCodeHelper.GetModelCodeFromString(userModelCode, out modelCode))
                {
                    ...
                    else if (!string.IsNullOrWhiteSpace(userModelCode))
                    {
                        modelCode = (ModelCode)long.Parse(userModelCode);
                    }
                    else
